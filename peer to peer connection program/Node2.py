...
def serverMode(name):
	...
	try:
	#Binding server Socket on port=9002
		serversocket.bind(('127.0.0.1',9002))
		...
		while(True):
			...
			if leader == int(myID[0]):
				...
				path='/home/rahul/Desktop/CN/CN_ASSIGNMENT/59_ASSIGNMENT2/NODE2/'
				log=[(path,'Node2_file.txt',serversocket)]
				...
				print('Connection between Node2 and Node3 is closed...')				
				clientsocket2.shutdown(SHUT_WR)
			elif leader == peer_lead[2]:
				clientsocket1.shutdown(SHUT_WR)
				print('Connection between Node2 and Node1 is closed...')	
...
def clientMode(name):
	...
	print('Node-2 sending its ID=2 to both Peer...')
	...
	if peer_lead[0] == int(myID[0]):	#If Node 2 is leader No need to break any connection.
		pass
	elif peer_lead[0] == peer_lead[1]:
		#If peer1 is leader, breaking connection between node2 and peer3.
		peer3_sock.close()
		print('Connection between Node2 and Node3 is closed...')	
		...
		peer1_sock.sendall('Node2_file.txt\r\n\r\n'.encode())		
		path='/home/rahul/Desktop/CN/CN_ASSIGNMENT/59_ASSIGNMENT2/NODE2/'
		with open(path+'Node2_file.txt') as f:
			while True:
				try:
					data=f.read(1024)
		...
		#If peer3 is leader, breaking connection between node2 and peer1.
		peer1_sock.close()  
		print('Connection between Node2 and Node1 is closed...')	
		...
		#Sending log data to leader
		peer3_sock.sendall('Node2_file.txt\r\n\r\n'.encode())	
		path='/home/rahul/Desktop/CN/CN_ASSIGNMENT/59_ASSIGNMENT2/NODE2/'
		with open(path+'Node2_file.txt') as f:
			while True:
				try:
